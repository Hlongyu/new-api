...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.ResponseModelName != "" {
		other["response_model_name"] = relayInfo.ResponseModelName
	}
...
func appendBillingInfo(relayInfo *relaycommon.RelayInfo, other map[string]interface{}) {
	...
	if relayInfo.SubscriptionQuota > 0 {
		other["subscription_consumed"] = relayInfo.SubscriptionQuota
	}
...
func InjectTieredBillingInfo(other map[string]interface{}, relayInfo *relaycommon.RelayInfo, result *billingexpr.TieredResult) {
	...
	if result != nil {
		...
		if len(result.ConditionalMultipliers) > 0 {
			other["request_multiplier"] = result.RequestMultiplier
			other["conditional_multipliers"] = result.ConditionalMultipliers
		}
	}
}
