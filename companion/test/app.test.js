...
async function request(baseUrl, pathname, {
  ...
  const headers = {}
  if (cookie) headers.Authorization = `Bearer ${cookie.replace(/^session=/, '')}`
...
test('复用 New API 会话管理参榜状态并自动完成赞助扣费', async () => {
  ...
  const client = {
    async getSessionUser(authorization) {
      const user = users[`session=${String(authorization).replace(/^Bearer /, '')}`]
      if (!user) {
  ...
  const config = {
    ...
    supportActivityStartTimestamp: Date.parse('2026-07-16T16:00:00.000Z') / 1000,
  ...
  try {
    ...
    const guestApi = await request(baseUrl, '/leaderboard/api/me', { cookie: '' })
    ...
    const guestModelsApi = await request(baseUrl, '/modelstatus/api/summary', { cookie: '' })
    ...
    const modelMetrics = await request(baseUrl, '/modelstatus/api/metrics?model=gpt-test')
    ...
    const rejectedLongName = await request(baseUrl, '/leaderboard/api/me', {
      method: 'PATCH',
      body: {
        displayName: '这是一个超过三十六个字符的展示名称用于验证后端限制确实生效请务必拒绝保存啊',
        isNamePublic: true,
      },
    })
    ...
    const board = await request(baseUrl, '/leaderboard/api/leaderboard?period=day')
    ...
    const sponsorBoard = await request(baseUrl, '/leaderboard/api/sponsors?period=all')
    ...
    assert.equal('supportPoints' in sponsoredUsageBoard.body.data.entries[0], false)
    assert.equal('supportTier' in sponsoredUsageBoard.body.data.entries[0], false)
    assert.equal('supportLit' in sponsoredUsageBoard.body.data.entries[0], false)
    assert.equal(typeof sponsoredUsageBoard.body.data.entries[0].rankLabel, 'string')
    assert.equal('rankScore' in sponsoredUsageBoard.body.data.entries[0], false)
    assert.equal('rankSegmentScore' in sponsoredUsageBoard.body.data.entries[0], false)
    assert.equal('inPromotion' in sponsoredUsageBoard.body.data.entries[0], false)
    ...
    application.db.finishSponsorOrder('old-sponsor-order', 'completed', '', oldCompletedAt)
    ...
    assert.match(anonymousUsageBoard.body.data.entries[0].displayName, /^匿名用户 /)
    ...
    const deniedSponsors = await request(baseUrl, '/leaderboard/api/admin/sponsors')
    assert.equal(deniedSponsors.status, 403)
    const rootSponsors = await request(baseUrl, '/leaderboard/api/admin/sponsors', {
      cookie: 'session=root',
      userId: 1,
    })
...
test('每周消费金额榜前三可按名次奖池抽奖，历史机会可累计领取', async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'leaderboard-lottery-'))
  const grants = []
  let grantMode = 'complete'
  const users = {
    'session=alice': { id: 42, username: 'alice', display_name: 'Alice', quota: 1 },
    'session=bob': { id: 43, username: 'bob', display_name: 'Bob', quota: 1 },
    'session=carol': { id: 44, username: 'carol', display_name: 'Carol', quota: 1 },
  }
  const client = {
    async getSessionUser(authorization) {
      const user = users[`session=${String(authorization).replace(/^Bearer /, '')}`]
  ...
  try {
    ...
    await request(baseUrl, '/leaderboard/api/me', { cookie: 'session=bob', userId: 43 })
    await request(baseUrl, '/leaderboard/api/me', { cookie: 'session=carol', userId: 44 })
    ...
    for (const [userId, tokenUsed, quota] of [
      ...
      for (const [periodType, key] of [['all', 'all'], ['week', weekKey]]) {
        application.db.upsertAggregate({
          entryId: entry.id,
          periodType,
          periodKey: key,
    ...
    assert.equal(carolDraw.status, 201)
    assert.equal(carolDraw.body.data.status, 'completed')
    ...
    const aliceSecondDrawn = await request(baseUrl, '/leaderboard/api/lottery/draw', {
      method: 'POST',
      body: {},
    })
    ...
    assert.equal(drawnView.body.data.history.length, 3)
    assert.deepEqual(
      drawnView.body.data.history.map((item) => [item.rank, item.amountUsd]),
      [[1, 5], [2, 2], [1, 5]],
    )
    const historyNames = drawnView.body.data.history.map((item) => item.displayName)
    assert.equal(historyNames[0], 'Alice Draw')
    assert.match(historyNames[1], /^匿名用户 /)
    assert.equal(historyNames[2], 'Alice Draw')
  } finally {
    await new Promise((resolve) => application.server.close(resolve))
    application.close()
    fs.rmSync(directory, { recursive: true, force: true })
...
test('用户可分榜单隐藏，周榜抽奖按真实消费计算，改名卡控制额外改名', async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'leaderboard-privacy-'))
  const deductions = []
  const users = {
    'session=alice': {
      id: 42,
  ...
  try {
    ...
    const entry = application.db.getEntryByUserId(42)
    const now = new Date()
    const previousWeekKey = previousWeekRange({ timeZone: config.timeZone }).key
    for (const [periodType, key] of [
      ['all', 'all'],
      ['day', periodKey('day', { now, timeZone: config.timeZone })],
    ...
    const dayBoard = await request(baseUrl, '/leaderboard/api/leaderboard?period=day')
    ...
    const weekBoard = await request(baseUrl, '/leaderboard/api/leaderboard?period=week')
    ...
    const duplicateCardOrder = await request(baseUrl, '/leaderboard/api/rename-cards', {
      method: 'POST',
      body: { requestKey: 'rename_card_request_1', quantity: 2 },
    })
    ...
    const insufficientCardOrder = await request(baseUrl, '/leaderboard/api/rename-cards', {
      method: 'POST',
    ...
    assert.equal(deductions.length, 1)
    assert.equal(application.db.getRenameCardOrderByRequestKey('rename_card_request_2'), null)
